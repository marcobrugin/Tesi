...
Dopo aver configurato i \gls{cluster}{} sopra descritti, e aver creato una \gls{Data Pipeline}{} come illustrato nella sezione \ref{sec:approccio_utilizzato} ho creato dei test 
...
Per poter generare i dati necessari al \gls{datasource}{} sopra descritto, è stato utilizzato il produttore di eventi, sopra descritto e la libreria \gls{Faker}{}, utilizzando i seguenti metodi (per motivi di spazio viene riportata solo la generazione 
dei campi nome e cognome della \gls{datasource}{}).
\begin{lstlisting}[language=Python, caption=\texttt{producer.py}, label=lst:file]{producer.py}
fake = Faker()
...
sono stati salvati in un file \textbf{.csv} e successivamente importati all'interno di \textbf{PostgreSQL}.\\
Dopo la creazione del \gls{datasource}{} è stato eseguita l'\gls{injection}{} all'interno di \textbf{Apache Druid} 
andando a utilizzare l'interfaccia web, reperibile all'indirizzo\\
\textbf{http://localhost:8888/} (Figura \ref{fig:load_data}).
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{images/percorso/load_data.png}
    \caption{Injection del datasource all'interno di Apache Druid}
    \label{fig:load_data}
\end{figure}
\pagebreak
\\
In seguito sono state eseguite le seguenti query, su entrambi sia su \textbf{Apache Druid} che su \textbf{PostgreSQL}.
...
Inoltre si può notare anche che il divario tra le prestazioni aumenta nel momento 
in cui si eseguono query che coinvolgono operazioni su timestamp primario, infatti i dati vengono archiviati secondo un partizionamento temporale definito 
al momento dell'\gls{injection}{} (vedi \ref{sec:deep_storage})
\pagebreak
\subsubsection{Confronto delle prestazioni di esecuzione tra datasource con e senza rollup in Apache Druid}
Con il termine di \textbf{Data rollup} si intende un'operazione di aggregazione 
dei dati, che permette di ridurre la dimensione dei dati archiviati, andando a creare dei record di archiviazione più piccoli e migliorando così 
le prestazioni di esecuzione delle query.\\
In \textbf{Apache Druid} l'operazione di \textbf{rollup}, viene eseguita attraverso la 
definizione di segmenti aggregati, creati in base a delle regole di aggregazione definite al momento dell'\gls{injection}{} del \gls{datasource}{}, direttamente dall'interfaccia web di \textbf{Apache Druid} (Figura \ref{fig:rollup}).
\begin{figure}[h]
  \centering
  ...
  \label{fig:rollup}
\end{figure}
In generale l'operazione di \textbf{rollup} può essere configurata in 
È importante sottolineare che l'operazione di \textbf{rollup} può essere configurata in base 
alla granularità temporale: si possono andare ad aggregare segmenti aventi, la stessa ora, minuti o secondo e
in seguito andare a considerare i valori degli altri campi.\\
Per mettere in risalto la funzionalità appena descritta ho creato un \gls{datasource}{} di cinque milioni di record, a partire dal seguente modello relazione.
\begin{lstlisting}[language=SQL]
  CREATE TABLE accessi(__time timestamp, nome text, cognome text, citta text, stato text, datan date, istruzione text, hobby text)
\end{lstlisting}
In questo caso si tratta di un modello relazionale molto semplice 
e per riuscire ad apprezzare la funzionalità di \textbf{rollup} ho ridotto
...
è stato utilizzato il produttore di eventi, sopra descritto e la libreria \gls{Faker}{}, nel seguente modo.
\begin{lstlisting}[language=Python, caption=\texttt{producer.py}, label=lst:file]{producer.py}
  fake = Faker()
  ...
  for n in range(50):
    ...
    \end{lstlisting}
In seguito, dopo aver salvato tutti i dati generati, sono state eseguenti le seguenti query su i tre \gls{datasource}{} creati.
\begin{lstlisting}[language=SQL]
  - query 1: SELECT DATE_TRUNC('day', __time), citta, COUNT(*)
              FROM accessi
              GROUP BY DATE_TRUNC('day', __time), citta
...
Infatti se si confronta le cardinalità dei due \gls{datasource}{} con e senza \textbf{rollup} si nota che da cinque milioni di record iniziali
si arriva a centocinquantamila record aggregati, che permettendo così una riduzione considerevole del volume di dati da analizzare.\\
...
classico database relazione, come \textbf{PostgreSQL}.
\pagebreak
