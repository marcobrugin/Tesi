...
\begin{lstlisting}[caption=\texttt{enviroment}, label=lst:file1]{enviroment}
...
Dopo aver configurato il file di enviroment per \textbf{Apache Druid} è stato configurato il \gls{cluster}{} di \textbf{Apache Druid} e le risorse necesssarie, utilizzando il file \ref{lst:file2} con \textbf{Docker Compose}.\\
In questo caso per ottimizzare il consumo di memoria RAM sono stati utilizzati un nodo per \textbf{coordinator}, \textbf{Historical}, \textbf{Broker} e \textbf{MiddleManager}, implementato con \textbf{PostgreSQL}.\\
Inoltre per la gestione del \gls{cluster}{} è stato riutilizzato il nodo \textbf{Zookeeper} precedentemente configurato.
\begin{lstlisting}[caption=\texttt{druid-cluster-compose.yml}, label=lst:file2]{druid-cluster-compose.yml}
networks:
  kafka-druid:
    name: kafka-druid
    driver: bridge
