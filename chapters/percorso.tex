...
record per effettuare in seguito la creazione della tabella di \textbf{lookup} (Figura \ref{fig:lookup}).\\
\begin{figure}[h]
  \centering
...
si può notare le pogtenzialità che offrono le tabelle di \textbf{lookup} di \textbf{Apache Druid}.\\
Inoltre si sottolinea che per ottenere lo stesso risultato all'interno di un classico \textbf{database relazionale}
